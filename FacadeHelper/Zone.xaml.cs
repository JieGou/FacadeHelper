...
namespace FacadeHelper
{
    ...
    public partial class Zone : UserControl, INotifyPropertyChanged
    {
        ...
        public int CurrentZonePanelType
        {
            get => _currentZonePanelType;
            set
            {
                _currentZonePanelType = value;
                OnPropertyChanged(nameof(CurrentZonePanelType));
            }
        }
        ...
        public int CurrentZoneLevel
        {
            ...
            set
            {
                ...
                OnPropertyChanged(nameof(CurrentZoneLevel));
            }
        }

        public string CurrentZoneDirection
        {
            get => _currentZoneDirection;
            set
            {
                _currentZoneDirection = value;
                OnPropertyChanged(nameof(CurrentZoneDirection));
            }
        }

        public string CurrentZoneSystem
        {
            get => _currentZoneSystem;
            set
            {
                _currentZoneSystem = value;
                OnPropertyChanged(nameof(CurrentZoneSystem));
            }
        }
        ...
        public int CurrentZoneIndex
        {
            ...
            set
            {
                ...
                OnPropertyChanged(nameof(CurrentZoneIndex));
            }
        }

        public string CurrentZoneCode
        {
            get => _currentZoneCode;
            set
            {
                _currentZoneCode = value;
                OnPropertyChanged(nameof(CurrentZoneCode));
            }
        }
        ...
        public bool IsSearchRangeZone
        {
            get => _isSearchRangeZone;
            set
            {
                _isSearchRangeZone = value;
                OnPropertyChanged(nameof(IsSearchRangeZone));
            }
        }

        public bool IsSearchRangePanel
        {
            get => _isSearchRangePanel;
            set
            {
                _isSearchRangePanel = value;
                OnPropertyChanged(nameof(IsSearchRangePanel));
            }
        }

        public bool IsSearchRangeElement
        {
            get => _isSearchRangeElement;
            set
            {
                _isSearchRangeElement = value;
                OnPropertyChanged(nameof(IsSearchRangeElement));
            }
        }

        public bool? IsSearchRangeAll
        {
            get => _isSearchRangeAll;
            set
            {
                _isSearchRangeAll = value;
                OnPropertyChanged(nameof(IsSearchRangeAll));
            }
        }

        private bool _isRealTimeProgress = true;

        public bool IsRealTimeProgress
        {
            get => _isRealTimeProgress;
            set
            {
                _isRealTimeProgress = value;
                OnPropertyChanged(nameof(IsRealTimeProgress));
            }
        }

        private bool _isPidInitialized = false;

        public bool IsPidInitialized
        {
            get => _isPidInitialized;
            set
            {
                _isPidInitialized = value;
                OnPropertyChanged(nameof(IsPidInitialized));
            }
        }

        private bool _isElementIndexRangeInitialized = false;

        public bool IsElementIndexRangeInitialized
        {
            get => _isElementIndexRangeInitialized;
            set
            {
                _isElementIndexRangeInitialized = value;
                OnPropertyChanged(nameof(IsElementIndexRangeInitialized));
            }
        }

        private bool _isZoneLayerInitialized = false;

        public bool IsZoneLayerInitialized
        {
            get => _isZoneLayerInitialized;
            set
            {
                _isZoneLayerInitialized = value;
                OnPropertyChanged(nameof(IsZoneLayerInitialized));
            }
        }

        private bool _isElementClassInitialized = false;

        public bool IsElementClassInitialized
        {
            get => _isElementClassInitialized;
            set
            {
                _isElementClassInitialized = value;
                OnPropertyChanged(nameof(IsElementClassInitialized));
            }
        }

        private bool _isExteriorDataLinked = false;

        public bool IsExteriorDataLinked
        {
            get => _isExteriorDataLinked;
            set
            {
                _isExteriorDataLinked = value;
                OnPropertyChanged(nameof(IsExteriorDataLinked));
            }
        }

        private int _countExteriorDataLinked = 0;

        public int CountExteriorDataLinked
        {
            get => _countExteriorDataLinked;
            set
            {
                _countExteriorDataLinked = value;
                OnPropertyChanged(nameof(CountExteriorDataLinked));
            }
        }
        ...
        public int CurrentCultureIndex
        {
            ...
            set
            {
                _currentCultureIndex = value;
                OnPropertyChanged(nameof(CurrentCultureIndex));
            }
        }
        ...
        public Zone(ExternalCommandData commandData)
        {
            ...
            #endregion 检测 ElementIndexRange 是否加载
            ...
            if (eclist is null)
            {
                IsElementClassInitialized = false;
            }
            else
            {
            ...
            #endregion 检测 ElementClass 是否加载
            ...
            #endregion 检测 ZoneLayer 是否加载

            //确认PID/分区表/类型表都已加载才可允许工具条
            if (!(IsPidInitialized && IsZoneLayerInitialized && IsElementClassInitialized))
            {
                panelCommandBar.IsEnabled = false;
            }
        ...
        private void InitializeCommand()
        {
            ...
            CommandBinding cbT24Fix = new CommandBinding(cmdT24Fix, (sender, e) =>
            {
                ...
                Global.DocContent.CurtainPanelList.ForEach(p =>
                {
                    ...
                    foreach (ElementId eid in p_subs)
                    {
                        ...
                        _parameter = __element.LookupParameter/*get_Parameter*/("分项");
            ...
            CommandBinding cbElementClassify = new CommandBinding(cmdElementClassify,
                cbElementClassify_Executed,
                (sender, e) =>
                            {
                ...
                (sender, e) =>
                        {
                            ...
                            if (ofd.ShowDialog() == true)
                            {
                                ...
                                listInformation.SelectedIndex = listInformation.Items
                                    .Add($"{DateTime.Now:HH:mm:ss} - LINK: FILE, {string.Join(",", ofd.SafeFileNames)}.");
                ...
                (sender, e) =>
                        {
                            ...
                            if (Global.DocContent.FullZoneList.Count == 0)
                            {
                                foreach (var z in Global.DocContent.ZoneList)
                                {
                                    Global.DocContent.FullZoneList.Add(z);
                                }
                            }
            ...
            var elements = _uidoc.Selection.GetElementIds()
                .Select(id => _doc.GetElement(id)).ToList()/*Elements*/;
            ...
            (sender, e) =>
                        {
                            ...
                            listInformation.SelectedIndex = listInformation.Items
                                .Add($"{DateTime.Now:HH:mm:ss} - LIST: E/{_ele_asm.Count}/DEEP.");
                            ...
                            _ele_asm.ForEach(ae =>
                            {
                                ...
                                if (_deepids != null && _deepids.Count > 0)
                                {
                                    ...
                                    foreach (ElementId sid in _deepids)
                                    {
                                        ...
                                        if (_parameter != null)
                                        {
                                            if ((_parameter = __element.LookupParameter/*get_Parameter*/("分项")).HasValue)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    listInformation.SelectedIndex = listInformation.Items
                                                        .Add($"{DateTime.Now:HH:mm:ss} - ERR: VALUE TYPE, PARAM/TYPE, P/{_sei.INF_HostCurtainPanel.INF_ElementId}, E/{_sei.INF_ElementId}, {_sei.INF_Name}.");
                                            ...
                                            else
                                            {
                                                ...
                                                listInformation.SelectedIndex = listInformation.Items
                                                    .Add($"{DateTime.Now:HH:mm:ss} - ERR: NO VALUE TYPE, PARAM/TYPE, P/{_sei.INF_HostCurtainPanel.INF_ElementId}, E/{_sei.INF_ElementId}, {_sei.INF_Name}.");
                                                elements.Add(__element);
                                                continue;
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            listInformation.SelectedIndex = listInformation.Items
                                                .Add($"{DateTime.Now:HH:mm:ss} - ERR: PARAM NOTSET, PARAM/TYPE, P/{_sei.INF_HostCurtainPanel.INF_ElementId}, {_sei.INF_ElementId}.");
                                            elements.Add(__element);
                                            continue;
                                        }
                            ...
                            _ele_asm.ForEach(ae =>
                            {
                                ...
                                if (_deepids != null && _deepids.Count > 0)
                                {
                                    ...
                                    foreach (ElementId sid in _deepids)
                                    {
                                        ...
                                        Parameter _parameter;
                                        _parameter = __element.LookupParameter("分项");
                                        if (_parameter != null)
                                        {
                                            if ((_parameter = __element.LookupParameter("分项")).HasValue)
                                            {
                                                if (int.TryParse(_parameter.AsString(), out int _type))
                                                {
                                                    ElementClass __sec;
                ...
                (sender, e) =>
                        {
                            ...
                            listInformation.SelectedIndex = listInformation.Items
                                .Add($"{DateTime.Now:HH:mm:ss} - RESET: ALL DATA.");
                        },
                (sender, e) =>
                            {
                                e.CanExecute = true;
                ...
                (sender, e) =>
                        {
                            ...
                            if (!IsSearchRangeZone && !IsSearchRangePanel && !IsSearchRangeElement)
                            {
                                return;
                            }
                ...
                (sender, e) =>
                        {
                                ...
                                MessageBoxResult.Yes) == MessageBoxResult.No)
                            {
                                ...
                                {
                                    listInformation.SelectedIndex = listInformation.Items
                                        .Add($"{DateTime.Now:HH:mm:ss} - READ: DATA, {ofd.FileName}");
                                    using (Stream stream = new FileStream(ofd.FileName, FileMode.Open, FileAccess.Read, FileShare.Read))
                                    {
                                        ...
                                        listInformation.SelectedIndex = listInformation.Items
                                            .Add($"{DateTime.Now:HH:mm:ss} - UPDATE: LIST/P");
                                        listInformation.SelectedIndex = listInformation.Items
                                            .Add($"{DateTime.Now:HH:mm:ss} - UPDATE: LIST/E");
                                    }
                                }
                            }
                            ...
                            //try
                            {
                                using (Transaction trans = new Transaction(_doc, "Apply_Parameters_CurtainPanels"))
                                {
                                    ...
                                    Global.DocContent.CurtainPanelList.ForEach(p =>
                                    {
                                        ...
                                        if (_element != null)
                                        {
                                            _element.LookupParameter("立面朝向").Set(p.INF_Direction);
                                            _element.LookupParameter("立面系统").Set(p.INF_System);
                                            _element.LookupParameter("立面楼层").Set(p.INF_Level);
                                            _element.LookupParameter("构件分项").Set(p.INF_Type);
                                            _element.LookupParameter("分区序号").Set(p.INF_ZoneIndex);
                                            _element.LookupParameter("分区区号").Set(p.INF_ZoneCode);
                                            _element.LookupParameter("分区编码").Set(p.INF_Code);
                                        }
                                ...
                                listInformation.SelectedIndex = listInformation.Items
                                    .Add($"{DateTime.Now:HH:mm:ss} - WRITE: PARAM, P/{Global.DocContent.CurtainPanelList.Count}, Param#{Global.DocContent.CurtainPanelList.Count * 7}");
                                ...
                                using (Transaction trans = new Transaction(_doc, "Apply_Parameters_ScheduleElements"))
                                {
                                    ...
                                    Global.DocContent.ScheduleElementList.ForEach(ele =>
                                    {
                                        ...
                                        if (_element != null)
                                        {
                                            _element.LookupParameter("立面朝向").Set(ele.INF_Direction);
                                            _element.LookupParameter("立面系统").Set(ele.INF_System);
                                            _element.LookupParameter("立面楼层").Set(ele.INF_Level);
                                            _element.LookupParameter("构件分项").Set(ele.INF_Type);
                                            _element.LookupParameter("分区序号").Set(ele.INF_ZoneIndex);
                                            _element.LookupParameter("分区区号").Set(ele.INF_ZoneCode);
                                            _element.LookupParameter("分区编码").Set(ele.INF_Code);
                                        }

                                        if (IsRealTimeProgress)
                                        {
                                            txtCurrentProcessElement.Content =
                                                $"P/{ele.INF_HostCurtainPanel.INF_Code}.E/{ele.INF_ElementId}, {ele.INF_Code}";
                                            txtCurrentProcessOperation.Content = "W/PARAM";
                                            progbarCurrentProcess.Value++;
                                ...
                                using (Transaction trans = new Transaction(_doc, "Apply_Parameters_DeepElements"))
                                {
                                    ...
                                    if (!Global.DocContent.ParameterInfoList.Exists(x => x.Name == "组件编码"))
                                    {
                                        ...
                                        ParameterHelper
                                            .RawCreateProjectParameter(_doc.Application, "组件编码",
                                                ParameterType.Text, true, _catset, BuiltInParameterGroup.PG_DATA, true);
                                    }

                                    #endregion 设置项目参数：组件编码
                                    ...
                                    Global.DocContent.DeepElementList.ForEach(ele =>
                                    {
                                        ...
                                        _element.LookupParameter("分区区号").Set(ele.INF_ZoneCode);
                                        _element.LookupParameter("分区编码").Set(ele.INF_Code);
                                        _element.LookupParameter("组件编码").Set(ele.INF_HostScheduleElement.INF_Code);

                                        if (IsRealTimeProgress)
                                        {
                                            txtCurrentProcessElement.Content =
                                                $"E/{ele.INF_HostScheduleElement.INF_Code}.DEEP-E/{ele.INF_ElementId}, {ele.INF_Code}";
                                            txtCurrentProcessOperation.Content = "W/PARAM";
                                            progbarCurrentProcess.Value++;
                                ...
                                listInformation.SelectedIndex = listInformation.Items
                                    .Add($"{DateTime.Now:HH:mm:ss} - WRITE: PARAM, E/{Global.DocContent.ScheduleElementList.Count}, Param#{Global.DocContent.ScheduleElementList.Count * 7}");
                            }
                            //catch (Exception ex)
                            {
            ...
            CommandBinding cbExportElementSchedule = new CommandBinding(cmdExportElementSchedule,
                (sender, e) =>
                        {
                            using (StreamWriter writer = new StreamWriter(System.IO.Path
                                .Combine(System.IO.Path.GetDirectoryName(_doc.PathName),
                                    $"{System.IO.Path.GetFileNameWithoutExtension(_doc.PathName)}.4dzone.csv"), false))
                            {
                                int idtask = 0;
                                ...
                                foreach (var zone in Global.DocContent.ZoneList)
                                {
                                    ...
                                    foreach (var m in _elist)
                                    {
                                        writer.WriteLine($"{++idtask},{m.INF_Code},{m.INF_TaskStart},{m.INF_TaskFinish},2");
                                    }
                                }
                            }
                        },
        ...
        private void cbElementClassify_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            using (Transaction trans = new Transaction(_doc, "CreatePanelGroup"))
            {
                ...
                {
                    selectset = SelectionFilterElement.Create(_doc, _currentZoneInfo.ZoneCode);
                }
        ...
        #endregion Command -- bnElementClassify : 嵌板和构件归类
        ...
        #endregion Command -- bnModelInit : 模型初始化
        ...
        private void InitModelData()
        {
            ...
            //统计
            LogicalAndFilter cwpanel_InstancesFilter =
                new LogicalAndFilter(new ElementClassFilter(typeof(FamilyInstance)),
                    new ElementCategoryFilter(BuiltInCategory.OST_CurtainWallPanels));
            LogicalAndFilter win_InstancesFilter =
                new LogicalAndFilter(new ElementClassFilter(typeof(FamilyInstance)),
                    new ElementCategoryFilter(BuiltInCategory.OST_Windows));
            var panels = new FilteredElementCollector(_doc).WherePasses(cwpanel_InstancesFilter)
                .Where(x => (x as FamilyInstance).Symbol.Family.Name != "系统嵌板"
                            && (x as FamilyInstance).Symbol.Name != "空嵌板"
                            && (x as FamilyInstance).Symbol.Name != "空系统嵌板");
            var wins = new FilteredElementCollector(_doc).WherePasses(win_InstancesFilter).ToElements();
            var syspanels = new FilteredElementCollector(_doc)
            ...
            int nwele = 0;
            int nsysele = 0;
            foreach (var _p in panels)
            {
                nele += (_p as FamilyInstance).GetSubComponentIds().Count;
            }

            foreach (var _w in wins)
            {
                nwele += (_w as FamilyInstance).GetSubComponentIds().Count;
            }

            foreach (var _p in syspanels)
            {
                nsysele += (_p as FamilyInstance).GetSubComponentIds().Count;
            }

            listInformation.SelectedIndex = listInformation.Items
                .Add($"{DateTime.Now:HH:mm:ss} - CALC: TOTAL, P/{panels.Count():N0}," +
                     $" E/{nele:N0}, SYS-P/{syspanels.Count():N0}," +
                     $" SYS-E/{nsysele:N0}");
            listInformation.SelectedIndex = listInformation.Items
                .Add($"{DateTime.Now:HH:mm:ss} - CALC: TOTAL, W/{wins.Count():N0}, E/{nwele:N0}");
        }

        #endregion Function 初始化模型数据

        #endregion 初始化 Command
        ...
        private void Chkbox_Checked(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                if (IsSearchRangeZone && IsSearchRangePanel && IsSearchRangeElement)
                {
                    IsSearchRangeAll = true;
                }
                else if (!IsSearchRangeZone && !IsSearchRangePanel && !IsSearchRangeElement)
                {
                    IsSearchRangeAll = false;
                }
                else
                {
                    IsSearchRangeAll = null;
                }
            }
        ...
        private void datagridElements_GotFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (datagridScheduleElements.CurrentItem != null)
            {
                currentsei = (ScheduleElementInfo)(datagridScheduleElements.CurrentItem);
            }
        }
    }
